...
AvgRecovery = (AverageLeadRecovery+AverageLaggRecovery)/2;
...
if UseAverageLeadLagg
    ...
    AverageLeadRecovery = AvgRecovery;
    AverageLaggRecovery = AvgRecovery;
else
...
for mmhStyle = 3
    ...
    for Samplings=TotalSamples
        ...
        FLagg=1-BLagg;
        runningMMH=1;
        myfilename=sprintf('MMH_FRAP_Dynamic_dt1_SymmetricLeadLagg_Average%d/Run_%g.mat',UseAverageLeadLagg, Samplings);
        ...
        time=0;
        while runningODE
            [~,y1Lead] = ode15s(@(t,y0Lead)ODE_DL_FRAP_Dynamic_Symmetric(t,ParametersLead,y0Lead),[time-0.1*deltaT,time],y0Lead);
            y1Lead=y1Lead(end,:);
            [~,y1Lagg] = ode15s(@(t,y0Lagg)ODE_DL_FRAP_Dynamic_Symmetric(t,ParametersLagg,y0Lagg),[time-0.1*deltaT,time],y0Lagg);
            y1Lagg=y1Lagg(end,:);
            t=t+1;
            time=time+.1*deltaT;
            if time>=t_max-1
        ...
        if ~BadParameters
            ...
            parsave(Samplings, array,hits,DistanceSimData,j,UseAverageLeadLagg)
            ...
            parsave(Samplings, array,hits,[0 0 0 0 0],j,UseAverageLeadLagg)
        end
        ...
        while runningMMH
            ...
            time=0;
            while runningODE
                [~,y1Lead] = ode15s(@(t,y0Lead)ODE_DL_FRAP_Dynamic_Symmetric(t,NewParametersLead,y0Lead),[time-(0.1*deltaT),time],y0Lead);
                y1Lead=y1Lead(end,:);
                [~,y1Lagg] = ode15s(@(t,y0Lagg)ODE_DL_FRAP_Dynamic_Symmetric(t,NewParametersLagg,y0Lagg),[time-(0.1*deltaT),time],y0Lagg);
                y1Lagg=y1Lagg(end,:);
        ...
        parsave(Samplings, array,hits,hits(end,end-4:end),j,UseAverageLeadLagg)

    end
end

function parsave(Samplings, array,hits,DistanceSimData,j,UseAverageLeadLagg)
save(sprintf('MMH_FRAP_Dynamic_dt1_SymmetricLeadLagg_Average%d/Run_%g.mat',UseAverageLeadLagg, Samplings),'array','hits','DistanceSimData','j')
end
