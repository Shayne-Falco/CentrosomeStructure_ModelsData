function candidate=mmh(array,proportion,nonneg,style)
...
for i=1:array_size
    ...
    if(nonneg==1 && candidate(i)<0) %check if non-negative values are not allowed
        candidate(i)=array(i);
    %elseif candidate(i)>6
        %candidate(i)=array(i);
    elseif style == 1 %original objective function
    ...
    elseif style == 2 %uniform objective function
        %reversibility condition
        pxi=1;
        qxi=1/(sqrt(2*pi)*proportion*array(i))*exp(-(candidate(i)-array(i))*(candidate(i)-array(i))/(2*(proportion*array(i))*(proportion*array(i))));
    ...
    end %if style equals 3 then no objective function
