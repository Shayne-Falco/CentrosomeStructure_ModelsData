...
mmhStyles = 1:3;
toDelete=0;
preLabel = 'Paper_Data/Hists/FitRecruit_Dynamic/';
...
for mmhStyle = mmhStyles
    for run=1:2000
        myfilename=['MMH_Dynamic_style_',num2str(mmhStyle),'/Run_',num2str(run), '.mat'];
...
N1mN2 = FinalParameters(:,8)-FinalParameters(:,9);
figure
hh =histogram(N1mN2,26,'BinLimits',[-1.75,1.75]);
xlabel('n_1 - n_2')
sum(N1mN2>0)/(sum(N1mN2>0)+sum(N1mN2<0))
figure
scatter(FinalParameters(:,8),FinalParameters(:,9))
xlim([1 3])
ylim([1 3])
[RHO,PVAL] = corr(FinalParameters(:,8),FinalParameters(:,9),'Type','Spearman');

HistBinEdges = hh.BinEdges;
HistBinEdges = HistBinEdges(1:end-1)+hh.BinWidth;
HistData = [HistBinEdges' hh.Values'];
filename = append('Paper_Data/Hists/FitRecruit_Dynamic/Histdata_n1mn2.dat');
    writematrix(HistData,string(filename),'filetype', 'text','Delimiter','tab')
